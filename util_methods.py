...
def ready_for_election(ports_of_all_nodes, self_election, self_coordinator):
    ...
    if True in election_array or True in coordinator_array:
        return False, node_details
    else:
        return True, node_details
...
def get_details(ports_of_all_nodes):
    ...
    for each_node in ports_of_all_nodes:
        ...
        except (NewConnectionError, ConnectionError, MaxRetryError) as err: 
            print("error connecting node %s" % ports_of_all_nodes[each_node])
            # print("error is %s", err)
    return node_details

# get detail of single node 
def get_single_detail(port): 
    url = 'http://localhost:%s/nodeDetails' % port
    node_details = []
    try: 
        data = requests.get(url)
        node_details.append(data.json())
    except (NewConnectionError, ConnectionError, MaxRetryError): 
        print("error connecting node %s" % port)
    if len(node_details) == 1: 
        return True
    return False

# this method is used to announce that it is the master to the other nodes.
def announce(coordinator, coorPort):
    ...
    for each_node in all_nodes:
        ...
        try: 
            ...
            print("error annoucing to node %s" % all_nodes[each_node])
            # print("error is %s", err)
